...
var reverse = function(string, index = 0) {
  let midPoint = Math.floor(string.length / 2);
  if (index === midPoint) {
    return string;
  } else {
    let strArr = string.split('');
    let copy = strArr[index];
    strArr[index] = strArr[strArr.length - index - 1];
    strArr[strArr.length - index - 1] = copy;
    return reverse(strArr.join(''), index + 1);
  }
};
...
var capitalizeFirst = function(input, array = [], index = 0) {
  if (index === input.length) {
    return array;
  } else {
    array.push(input[index][0].toUpperCase() + input[index].slice(1));
    return capitalizeFirst(input, array, index + 1);
  }
};
...
var numToText = function(str, result = []) {
  ...
  if (str.length === 0) {
    ...
    if (numObj.hasOwnProperty(strArr[0])) {
      result.push(numObj[strArr[0]]);
    } else {
      result.push(strArr[0]);
    }
    return numToText(strArr.slice(1).join(' '), result);
  }
};
